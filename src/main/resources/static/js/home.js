...
$('.input-sm').bind('input propertychange', function() {
	if ($(".input-sm").val() == "") {
		$("#input-videoSearch").attr("disabled", true);
	} else {
		$("#input-videoSearch").attr("disabled", false);
	}
	;
})
function query(search) {
	// console.log(search);
	table = $("#findAll").dataTable(datatables_options).api();
...
var datatables_options = {
	...
	/*
	 * createdRow: function ( row, data, index ) { if ( index %2 == 0 ) {
	 * $('td', row).css('font-weight',"bold").css("background-color","#cac1c1"); } },
	 */

	// 异步请求
	"ajax" : {
		"url" : oUrl,// "/act/findAll",
		"type" : "POST",
		"async" : false,
		"dataType" : "json",
		/* "cache": false, */
	...
	"columnDefs" : [
			...
			},
			{
				...
				"render" : function(data, type, full, meta) {
					...
					/*
					 * var btn = $("<button class = \"btn btn-default btn-sm
					 * btn-tmodal\" type=\"button\" data-toggle=\"modal\"
			...
			},
			{
				...
				"render" : function(data, type, full, meta) {
					var ret = "<button id="
							...
							+ " name="
							+ full.activityName
							+ " class = \"btn btn-primary btn-sm\" type=\"button\" onClick=\"to(this)\" ><span class=\"glyphicon glyphicon-list\"></span>详情</button>"
					return ret;
				}
			},
			{
...
$('#input-videoSearch').bind('input propertychange', function() {
	...
	/*
	 * table.clear().draw(); table =
	 * $('#findAll').DataTable(datatables_options); table.draw();
	 */

})

function ssOninput() {
	if ($("#findAll_filter .input-sm").val()) {
		$("#input-videoSearch").attr("disabled", true);
	} else {
		$("#input-videoSearch").attr("disabled", false);
	}
...
/*
 * $('#input-videoSearch').bind('input propertychange', function() { var tag =
 * this.value; $.ajax({ url : '/act/findActivityListByVideoTag',
 * //'/videoTag/findActivityByTag', type : 'post', async : false, dataType :
 * "json", data : ({ "tag" : tag }), dataFilter : function(data, type) {
 * $("#findAll").dataTable({aaData : data}); } });
 * table.ajax('/act/findActivityListByVideoTag').load(); });
 */
